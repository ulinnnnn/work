...
CprecordVO CprecordVO =CprecordSvc.findCprecordBycpr_No(number);
session.setAttribute("Cprecord_No", number);
session.setAttribute("CprecordVO", CprecordVO);
session.setAttribute("location", request.getServletPath()+"?"+number);
...
String cpstate= CproductSvc.getOneCproduct(CprecordVO.getCp_No1()).getCpState();
System.out.print( cpstate);
session.setAttribute("cpstate", cpstate);
String cpstate2= CproductSvc.getOneCproduct(CprecordVO.getCp_No2()).getCpState();
System.out.print( cpstate2);
session.setAttribute("cpstate2", cpstate2);
%>
<c:if test="${CprecordVO.cprstate eq '交換完成' }">
<% 
CproductSvc.changestate("交換完成",CproductSvc.getOneCproduct(CprecordVO.getCp_No1()).getCp_No());
System.out.print( "有跑道");
CproductSvc.changestate("交換完成",CproductSvc.getOneCproduct(CprecordVO.getCp_No2()).getCp_No());
%>
...
<c:if test="${cpstate eq '已上架'  or cpstate eq '交換申請中' or cpstate eq '待上架'  or cpstate2 eq '已上架'  or cpstate2 eq '交換申請中'  or cpstate2 eq '待上架'}">
<% CproductSvc.changestate("交換中",CproductSvc.getOneCproduct(CprecordVO.getCp_No1()).getCp_No());
CproductSvc.changestate("交換中",CproductSvc.getOneCproduct(CprecordVO.getCp_No2()).getCp_No());
CprecordSvc.changestate("交換中", number);
%>
</c:if>
<%pageContext.setAttribute("cprstate", CprecordVO.getCprstate());
%>



<c:if test="${cpstate eq '確定交換' and cpstate2 eq '確定交換'}">
<jsp:forward page="changeorder_4.jsp"/>
</c:if>

<c:if test="${cpstate eq '確定交換'  or cpstate2 eq '確定交換'}">
  ...
  <body  onload="connect();" onunload="disconnect();"  id="body" class="body-wrapper version1 up-scroll">
   ...
   <div style="overflow:visible;">
          ...
          <div class="row">
            <div class="col-12">
              <div class="btn-group" role="group" aria-label="...">
                 <a href="readrequest.jsp" class="btn btn-default "><i class="fa fa-th" aria-hidden="true"></i>交換申請</a>
                <a href="addchange.jsp" class="btn btn-default"><i class="fa fa-user" aria-hidden="true"></i>新增交換商品</a>
                <a href="readchange.jsp" class="btn btn-default"><i class="fa fa-map-marker" aria-hidden="true"></i>我的商品</a>
                <a href="readrecord.jsp" class="btn btn-default  active"><i class="fa fa-list" aria-hidden="true"></i>交換紀錄</a>
                <a href="readwish.jsp"class="btn btn-default"><i class="fa fa-gift" aria-hidden="true"></i>Wishlist</a>
              </div>
            </div>
          </div>
          <div class="row">
	...
	<div class="row progress-wizard" style="border-bottom:0;">
		...
		<div class="card-deck">
			<div class="card">
				...
				<div class="card-block">
			     ...
			     <input type="hidden" name="action" value="checkchange"></label></FORM>
			     <FORM  METHOD="post" ACTION="Cproduct.do" style="margin-bottom: 0px;">
					<label style="top:2px">
		...
		<!--			<button id="btn">${CprecordVO.mem_No2}</button><span style="display:none" id="toFriend">${CprecordVO.mem_No2}</span><span id="toFriendName">測試</span> -->
  ...
  <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
    <div class="modal-content">
        ...
        <div class="modal-header px-4">
          <h5 class="modal-title" id="exampleModalCenterTitle">修改之後將取消對方的確認狀態，請重新確認</h5>
        </div>
